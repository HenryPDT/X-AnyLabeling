...
class ModelManager(QObject):
    ...
    def _load_model(self, model_id):  # noqa: C901
        ...
        model_config["_on_progress"] = lambda downloaded, total: (
        ...
        if model_config["type"] == "yolov5":
            ...
            try:
                ...
                return
        elif model_config["type"] == "deepstream_det":
            from .deepstream_det import DeepStreamDetection

            try:
                model_config["model"] = DeepStreamDetection(
                    model_config, on_message=self.new_model_status.emit
                )
                self.auto_segmentation_model_unselected.emit()
                logger.info(
