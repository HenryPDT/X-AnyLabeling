...
### 5.5 数字快捷键管理器与快速数字标签绑定

<p align="center">
  <img src="../../assets/resources/digit_shortcut_manager.png" alt="用于给数字键分配绘制模式和默认标签的数字快捷键管理器">
</p>

数字快捷键管理器用于为数字键（`1`–`9`，`0`）配置绘制模式和默认标签。选择**工具 > 数字快捷键管理器**，或按 `Alt+D` 打开配置界面。

#### 快速数字标签绑定（自动 1–9, 0 映射）
当启用快速数字标签时（默认启用），数字键将根据项目类别列表顺序提供快速切换：
- **选中对象时**：按 `1` 立即将所有选中对象修改为第 1 个类别，按 `2` 修改为第 2 个类别，...，按 `9` 修改为第 9 个类别，按 `0` 修改为第 10 个类别，无需打开弹窗。完全支持撤销（`Ctrl+Z`）。
- **未选中对象时**：按下数字键会在“标签”停靠栏中高亮选中该类别，并预设为当前或下一次绘制的默认类别。

#### 自定义数字快捷键
在数字快捷键管理器对话框（`Alt+D`）中，用户可自定义指定数字键：
- **数字**：表示键盘上的数字键（0-9）
- **绘制模式**：从下拉菜单中选择矩形框、长方体、多边形、旋转框、四边形、圆形、线段、点、折线或“无”（仅绑定标签）
- **标签**：为该数字指定标签名称

**配置快捷键**：选择数字对应的绘制模式（或设为“无”仅绑定类别标签），输入标签名称，然后点击**确定**保存设置。若配置了绘制模式，未选中对象时按下该数字将自动切换至该绘制模式并预填标签。

**重置快捷键**：点击**重置**按钮可清除所有自定义快捷键设置，未分配的按键将自动回退为项目类别列表的顺序映射。
